...
type
  ...
  TComPortBaudRate = ( br110, br300, br600, br1200, br2400, br4800,
                       br9600, br14400, br19200, br28800, br38400, br56000,
  ...
  public
    ...
    property DSR: boolean read ReadDSR;
  published
...
function TCommPortDriver.CharCount: Integer;
var
  ComStat: TComStat;
  Errors: dword;
begin
...
function TCommPortDriver.ReadDSR : boolean;
var
  Stat: dword;
begin
  if not Connected then
  ...
  begin
    if GetCommModemStatus(FComPortHandle,Stat) <> false then
      Result := (Stat and MS_DSR_ON) <> 0
    else
      Result := false;
  end;
end;
...
const
  Win32BaudRates: array[br110..br115200] of DWORD =
    ( CBR_110, CBR_300, CBR_600, CBR_1200, CBR_2400, CBR_4800, CBR_9600,
      CBR_14400, CBR_19200, CBR_28800, CBR_38400, CBR_56000, CBR_57600, CBR_115200{v1.02 removed: CRB_128000, CBR_256000} );
  ...
  begin
    ...
    begin
      ...
      if nToSend > DataSize then
        nToSend := DataSize;
      { Send }
      if not WriteFile( FComPortHandle, DataPtr^, DataSize, dWord(nsent), nil ) then
        exit;
...
function TCommPortDriver.ReadChar( var c : char ): integer;
var ntoread, nread: DWord;
begin
  { 0 bytes sent }
  Result := 0;
  { Do nothing if not connected }
  ...
  ntoread := 1;
  if not ReadFile( FComPortHandle, c, ntoread, nread, nil ) and (nread = 1) then
    Result := 1
  else
    Result := 0;
end;
...
var nRead, dummy: dword;
    ...
    t: dword;
begin
  if (msg.Msg = WM_TIMER) and Connected and Assigned(FComPortReceiveData) then
  begin
