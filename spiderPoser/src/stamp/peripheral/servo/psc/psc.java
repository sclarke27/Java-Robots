...
public class psc {
   ...
   */
  public psc(Uart tx, Uart rx, int channels, int cpuPin) {
    this.tx = tx;
    this.rx = rx;
    this.cpuPin = cpuPin;
  ...
  public void initPsc(Uart pscIO) {
	StringBuffer tempStr = new StringBuffer();
	//Set PSC Baud rate to 38400
	CPU.delay(5000);               //Wait for servo controler to boot
	SerialLCD.clearLine(2);
	SerialLCD.writeLine("change baud to 38.4", 2);
	pscIO.sendString("!SCSBR");   //Set baud
	pscIO.sendByte(1);            //Set baud to 38400
	pscIO.sendByte(0x0D);
	...
	pscIO.restart(Uart.dirTransmit,cpuPin,Uart.dontInvert,Uart.speed38400,Uart.stop1);	    	
	CPU.delay(5000); 
	this.tx = pscIO;
	this.rx = pscIO;
  }  
