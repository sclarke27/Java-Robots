 ...
 *
 * The javelin connects to PSC via two pins:
 *
 ...
 * Date 12-dec-2005
 */
public class psc_twopin_test {

  static Uart pscTx = new Uart(Uart.dirTransmit,CPU.pin0,Uart.dontInvert,Uart.speed2400,Uart.stop1);
  static Uart pscRx = new Uart(Uart.dirReceive,CPU.pin1,Uart.dontInvert,Uart.speed2400,Uart.stop1);
  static psc myPsc = new psc(pscTx,pscRx,1); //single PSC board with 1 servo
  ...
  static void main() {
    int c,angle;
    System.out.println("PSC test program - two pin connection\n");
    initServos();
